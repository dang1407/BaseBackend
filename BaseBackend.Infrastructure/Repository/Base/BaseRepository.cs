...
namespace BaseBackend.Infrastructure
{
    ...
    {
        public async Task<BaseEntity> InsertItemAsync<TEntity>(BaseEntity entity) where TEntity : BaseEntity
        {
            ...
            int id = await unitOf.Connection.ExecuteScalarAsync<int>(query, param, transaction: unitOf.Transaction);
            idProperty?.SetValue(entity, id);
            return entity;
        }

        public BaseEntity InsertItem<TEntity>(TEntity entity) where TEntity : BaseEntity
        {
            var tableName = entity.GetTableName();
            List<PropertyInfo> _propertyInfo = typeof(TEntity).GetProperties()
                .Where(p => p.GetCustomAttributes(typeof(PropertyEntity), false)
            ...
            PropertyInfo? idProperty = typeof(TEntity).GetProperties().Where(p => p.GetCustomAttribute<PropertyEntity>()?.IsIdentity == true).FirstOrDefault();
            if (idProperty == null)
            {
                throw new Exception("Entity must have an identity column.");
            }
            ...
            // Thực thi truy vấn
            using UnitOfWork unitOf = new();
            int id = unitOf.Connection.ExecuteScalar<int>(query, param, transaction: unitOf.Transaction);
            idProperty?.SetValue(entity, id);
            return entity;
        }

        public async Task<int> UpdateItemAsync<TEntity>(TEntity entity) where TEntity : BaseEntity
        {
            var tableName = entity.GetTableName();
            List<PropertyInfo> _propertyInfo = typeof(TEntity).GetProperties()
                .Where(p => p.GetCustomAttributes(typeof(PropertyEntity), false)
        ...
        public int UpdateItem<TEntity>(TEntity entity) where TEntity : BaseEntity
        {
            var tableName = entity.GetTableName();
            List<PropertyInfo> _propertyInfo = typeof(TEntity).GetProperties()
                .Where(p => p.GetCustomAttributes(typeof(PropertyEntity), false)
        ...
        public async Task<int> DeleteItemByColumnName<TEntity>(BaseEntity entity, string colName, string colValue) where TEntity : BaseEntity
        {
            var tableName = entity.GetTableName();

            string query = $"delete from {tableName} where {colName} = @where_value";
